...
setTimeout(()=>{
  ...
  ok('incoming transfer named', body.includes('Transfer in') && body.includes('from #401'));
  ok('outgoing transfer named', body.includes('Transfer out') && body.includes('to #400'));
  ok('explains where stock is not coming from', body.includes('not the warehouse'));
  ok('outgoing says it is not going back to the warehouse', body.includes('not back to the warehouse'));
